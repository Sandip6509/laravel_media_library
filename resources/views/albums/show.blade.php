<x-app-layout>
    ...
    <div class="container mx-auto mt-2 p-2 bg-white shadow-md rounded-lg">
        <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
            <form method="post" action="{{ route('albums.upload',$album->id)}}" enctype="multipart/form-data">
                ...
                <div class="sm:col-span-6 pt-5">
                    <x-primary-button class="bg-green-600 font-semibold hover:bg-green-800">Upload</x-primary-button>
                </div>
            </form>
        </div>
        <div class="mt-4">
                ...
                @foreach ($photos as $photo)
                    <div class="bg-gray-300 p-2 rounded-lg">
                        <a class="block relative h-56 rounded overflow-hidden">
                            <img src="{{$photo->getUrl('thumb')}}" alt="gallery" class="object-cover object-center w-full h-full block"/>
                        </a>
                        <div class="flex justify-between mt-4 p-4">
                            <a class="m-2 p-2 bg-blue-500 hover:bg-blue-700 rounded-lg mr-2" href="{{ route('album.image.show',[$album->id,$photo->id])}}">Full Image</a>
                            <form action="{{route('album.image.destroy', [$album->id,$photo->id])}}" method="post">
                                @csrf
                                @method('DELETE')
                                <x-danger-button class="m-2 p-2 text-white bg-red-500 hover:bg-red-700 rounded-lg">Delete</x-danger-button>
                            </form>
                        </div>
                    </div>
                @endforeach
